...
<div class="modal">
    <div class="form" @keyup.esc="cancelEvent">
        ...
        <div v-if="mode=='update'"  class="form-group">
            ...
            <!-- 일련번호는 키이므로 disabled 처리 -->
        </div>
        <div class="form-group">
            <label>이름</label>
            <input type="text" name="name" class="long" v-model="contact.name" 
                ref="name" placeholder="이름을 입력하세요" @keyup.enter="submitEvent()"/>
        </div>
        <div class="form-group">
            <label>전화번호</label>
            <input type="text" name="tel" class="long" v-model="contact.tel" 
                placeholder="전화번호를 입력하세요" @keyup.enter="submitEvent()"/>
        </div>
        <div class="form-group">
            <label>주 소</label>
            <input type="text" name="address" class="long" v-model="contact.address" 
                placeholder="주소를 입력하세요"  @keyup.enter="submitEvent()"/>
        </div>
        <div class="form-group">
            <div>&nbsp;</div>
            <input type="button" class="btn btn-primary" 
                v-bind:value="btnText" @click="submitEvent()"  v-on:keyup.enter="submitEvent"/>
            <input type="button" class="btn btn-primary" 
